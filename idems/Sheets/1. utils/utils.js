...
function showErrors(sheet, headerRow) {
	...
	repeatedColumns = repeatedColumns.map((index) => sheet.getRange(1, index + 1).getA1Notation());

	let emptyColumns = getAllIndexes(headerRow, '');
	emptyColumns = emptyColumns.map((index) => sheet.getRange(1, index + 1).getA1Notation());

	const repeatedColumnsError = repeatedColumns.length ? `Repeated headings: ${repeatedColumns.join(', ')}` : null;
	const emptyColumnsError = emptyColumns.length ? `Empty headings: ${emptyColumns.join(', ')}` : null;
	...
	function getJoinedErrors(repeatedColumnsError, emptyColumnsError) {
		const initialError = 'Please fix the following errors in the header row: \n \n';
		let error = null;

		if (repeatedColumnsError && emptyColumnsError) error = repeatedColumnsError + '\n' + emptyColumnsError;
...
function moveDocToSubFolder(doc) {
	...
	// const subFolder = getOrCreateSubFolder(folder, spreadsheetName + ' attached docs');
