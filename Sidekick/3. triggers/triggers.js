...
class DynamicMenu {
  constructor(isShowInitialize = true) {
    this.createMenu = (ui) => {
      ...
      subMenu2.addItem("Create pull request", "openSidebar");
...
class DynamicMenuDocs {
  constructor() {
    this.createMenu = (ui) => {
      const menu = ui.createAddonMenu();

      const subMenu = ui.createMenu("Sync");
      subMenu.addItem("Sync with sheet", "syncDocWithSheetFORDocs");
...
function createUIDocs() {
  const menu = new DynamicMenuDocs();
